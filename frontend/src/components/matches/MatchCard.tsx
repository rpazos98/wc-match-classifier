import type { Match } from '../../types/index';
import { useAppState, useAppDispatch } from '../../state/AppContext';
import { fl } from '../../utils/flags';
import { scoreColor } from '../../utils/labels';

interface Props {
  match: Match;
}

export default function MatchCard({ match: m }: Props) {
  const { selectedId, simulated, hasLearned } = useAppState();
  const dispatch = useAppDispatch();

  const sel = m.match_id === selectedId ? ' sel' : '';
  const sc = scoreColor(m.score);

  const isKoSim = simulated && m.stage !== 'group';
  const showRare = m.rarity != null && m.rarity <= 0.1;

  const mDelta =
    hasLearned && m.base_score != null ? m.score - m.base_score : 0;
  const showDelta = hasLearned && Math.abs(mDelta) >= 1;

  const archTag = m.archetype?.label || null;
  const hasResult = m.home_goals != null && m.away_goals != null;

  const predLine = m.prediction ? (
    <span className="mc-pred">
      {fl(m.home)} {Math.round(m.prediction.p_home * 100)}%{' \u00b7 '}
      {Math.round(m.prediction.p_draw * 100)}%{' \u00b7 '}
      {Math.round(m.prediction.p_away * 100)}% {fl(m.away)}
    </span>
  ) : null;

  return (
    <div
      className={`match-card${sel}`}
      data-mid={m.match_id}
      onClick={() => dispatch({ type: 'SELECT_MATCH', id: m.match_id })}
    >
      <div className="mc-top">
        <span className="mc-score" style={{ color: sc }}>
          {m.score}
        </span>
        {showDelta && (
          <span
            style={{
              fontFamily: 'var(--font-mono)',
              fontSize: '9px',
              fontWeight: 700,
              color: mDelta > 0 ? '#3dd6c8' : '#e06060',
              marginLeft: '2px',
            }}
          >
            {mDelta > 0 ? '+' : ''}
            {mDelta.toFixed(0)}
          </span>
        )}
        <span className="mc-teams">
          {fl(m.home)} {m.home} vs {m.away} {fl(m.away)}
        </span>
        {isKoSim && (
          <span className="ko-sim-badge" title="Teams from simulation">
            sim
          </span>
        )}
        {showRare && (
          <span className="rare-badge">{Math.round(m.rarity! * 100)}%</span>
        )}
      </div>

      <div className="mc-mid">
        {archTag && <span className="mr-arch">{archTag}</span>}
        <span className="mc-meta">
          {m.stage_label} {'\u00b7'} {m.kickoff_local}
        </span>
      </div>

      {m.narrative && <div className="mc-narrative">{m.narrative}</div>}

      <div className="mc-bottom">
        {predLine}
        {hasResult && (
          <span className="mr-result">
            {m.home_goals} - {m.away_goals}
          </span>
        )}
        {m.predicted_winner && (
          <span className="mr-winner">
            <span className="arrow">{'\u25B6'}</span>
            {fl(m.predicted_winner)} {m.predicted_winner}
          </span>
        )}
      </div>
    </div>
  );
}
